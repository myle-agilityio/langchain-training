...
| 2h | Long-term memory (per-customer profile/tone via Store) | ✅ Done | On estimate — the Store was already wired in (pulled forward by the post-Phase-1 shared-inbox fix), so the work was profile shape + write tool + recall. Two bugs found by actually running it across two threads rather than typechecking: the model hallucinated the customer's email address when it was a tool argument (memory written to an unreachable key), and recall originally fired one model call *after* the draft it was supposed to shape. Both fixed and documented in ARCHITECTURE.md |
