...
See `ManageableStatusSchema` in `agent/src/tools/emails/tools.ts` — the model cannot mark an email
replied because the type has no such value, not because it was asked nicely.
