...
export function ExampleLayout({ chatContent, appContent }: ExampleLayoutProps) {
  ...
  useFrontendTool({
    name: "enableAppMode",
    description:
      "Enable app mode, make sure it's open when interacting with the inbox.",
    handler: async () => {
      setMode("app");
    },
  });
        ...
        className={`h-full overflow-hidden ${
          mode === "app"
            ? // Takes whatever the chat panel leaves, so dragging the divider is a single
              // width change. The dividing line itself lives in ResizeHandle.
              "flex-1 min-w-0"
            : "w-0 border-l-0"
        }`}
      >
        {/*
          Fill the state panel's own width. A viewport-relative width here would
          overflow this container when a drawer column is reserved.
        */}
